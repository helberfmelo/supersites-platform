...
$appPath = Join-Path $repoRoot 'apps/invoicecraft'
$serverPath = Join-Path $appPath '.output/server/index.mjs'

if (-not (Test-Path $serverPath)) {
    throw "Nuxt server build not found at $serverPath. Run pnpm --filter @supersites/invoicecraft build first."
}
...
try {
    ...
    $homeHtml = (Invoke-PreviewRequest -Uri "$baseUrl/en" -RequiredContent 'InvoiceCraft' -Attempts 40 -TimeoutSec 3).Content
    ...
    $tool = Invoke-PreviewRequest -Uri "$baseUrl/en/tools/invoice-builder" -RequiredContent 'Invoice Builder'
    if ($tool.StatusCode -ne 200 -or $tool.Content -notmatch 'FAQPage' -or $tool.Content -notmatch 'PDF') {
    ...
    if ($privacy.StatusCode -ne 200 -or $privacy.Content -notmatch 'InvoiceCraft privacy') {
        throw 'Privacy page smoke failed.'
    }

    $sitemap = Invoke-PreviewRequest -Uri "$baseUrl/sitemap.xml" -RequiredContent '<urlset'
    if ($sitemap.Content -notmatch '/en/privacy' -or $sitemap.Content -notmatch '/pt-br/tools/invoice-builder' -or $sitemap.Content -notmatch '/de/tools/receipt-builder') {
        throw 'Sitemap smoke failed.'
    }

    Write-Host "InvoiceCraft preview smoke passed on $baseUrl."
