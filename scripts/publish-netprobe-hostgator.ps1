...
RewriteRule ^(.+?)/?$ _netprobe-releases/{{RELEASE_ID}}/$1/index.html [L]
...
function New-PlaceholderRollbackHtaccessContent {
    return @'
# SuperSites managed rollback to NetProbe bootstrap placeholder.
RewriteEngine Off
'@
}

function Test-PublicApiGate {
    if ($SkipApiSmoke) {
        Write-Host "NetProbe public API smoke skipped by request."
        return
    }

    if ($script:ApiBaseUrl -notmatch "^https://") {
        throw "NetProbe API smoke requires HTTPS URL. Received: $script:ApiBaseUrl"
    }

    $headers = @{
        "User-Agent" = "NetProbeDeployPreflight/1.0"
        "Accept" = "application/json"
    }

    $ipUrl = "$($script:ApiBaseUrl.TrimEnd("/"))/ip"
    $ipResponse = Invoke-WebRequest -Uri $ipUrl -UseBasicParsing -TimeoutSec 30 -Headers $headers
    if ($ipResponse.StatusCode -ne 200 -or $ipResponse.Content -notmatch '"data"') {
        throw "NetProbe API preflight failed for $ipUrl."
    }
    if ($ipResponse.Content -match '(?i)<html|Internal Server Error|SuperSites bootstrap placeholder') {
        throw "NetProbe API preflight returned an HTML/error response for $ipUrl."
    }

    $dnsUrl = "$($script:ApiBaseUrl.TrimEnd("/"))/dns"
    $dnsResponse = Invoke-WebRequest -Uri $dnsUrl -UseBasicParsing -TimeoutSec 30 -Headers $headers -Method Post -ContentType "application/json" -Body '{"domain":"example.com","types":["A"]}'
    if ($dnsResponse.StatusCode -ne 200 -or $dnsResponse.Content -notmatch '"records"') {
        throw "NetProbe API preflight failed for $dnsUrl."
    }
    if ($dnsResponse.Content -match '(?i)<html|Internal Server Error|SuperSites bootstrap placeholder') {
        throw "NetProbe API preflight returned an HTML/error response for $dnsUrl."
    }

    Write-Host "NetProbe public API preflight passed for $script:ApiBaseUrl."
}

function Invoke-PublicSmoke {
    if ($SkipSmoke) {
        Write-Host "NetProbe public smoke skipped by request."
        return
    }

    $smokeScript = Join-Path $script:RepoRoot "scripts/smoke-netprobe-public.ps1"
    $smokeArgs = @{
...
function Switch-ManagedRelease {
    ...
    if (-not $releaseInfo -or -not $releaseInfo.exists) {
        throw "Cannot switch NetProbe release. Remote release folder was not found: $releasePath"
    }
...
$app = $manifest.apps | Where-Object { $_.id -eq "netprobe-atlas" } | Select-Object -First 1
if (-not $app) {
    throw "Deployment manifest does not contain the netprobe-atlas app."
}
...
$script:ReleaseBaseRemotePath = Join-RemotePath $script:RemoteBase "_netprobe-releases"
...
if (-not $SkipBuild) {
    & (Join-Path $script:RepoRoot "scripts/build-netprobe-hostgator-artifact.ps1") `
        -BasePath $basePath `
        -PublicBaseUrl $script:PublicBaseUrl `
        -ApiBaseUrl $script:ApiBaseUrl `
        -ReleaseId $release
    ...
    & (Join-Path $script:RepoRoot "scripts/validate-netprobe-static-artifact.ps1") `
...
Save-RemoteTextFile -Directory $releaseRemotePath -FileName "netprobe-release.json" -Content ($metadata | ConvertTo-Json -Depth 5)
Switch-ManagedRelease -ManagedReleaseId $release
Invoke-PublicSmoke

Write-Host "NetProbe HostGator deploy completed: release $release, $uploaded files, $totalBytes bytes."
