...
$appPath = Join-Path $repoRoot 'apps/qrroute'
$serverPath = Join-Path $appPath '.output/server/index.mjs'

if (-not (Test-Path $serverPath)) {
    throw "Nuxt server build not found at $serverPath. Run pnpm --filter @supersites/qrroute build first."
}
...
try {
    ...
    $homeHtml = (Invoke-PreviewRequest -Uri "$baseUrl/en" -RequiredContent 'QRRoute' -Attempts 40 -TimeoutSec 3).Content
    ...
    $tool = Invoke-PreviewRequest -Uri "$baseUrl/en/tools/utm-builder" -RequiredContent 'UTM Builder'
    if ($tool.StatusCode -ne 200 -or $tool.Content -notmatch 'FAQPage' -or $tool.Content -notmatch 'campaign URL') {
    ...
    if ($privacy.StatusCode -ne 200 -or $privacy.Content -notmatch 'QRRoute privacy') {
        throw 'Privacy page smoke failed.'
    }

    $sitemap = Invoke-PreviewRequest -Uri "$baseUrl/sitemap.xml" -RequiredContent '<urlset'
    if ($sitemap.Content -notmatch '/en/privacy' -or $sitemap.Content -notmatch '/pt-br/tools/utm-builder' -or $sitemap.Content -notmatch '/de/tools/wifi-qr') {
        throw 'Sitemap smoke failed.'
    }

    Write-Host "QRRoute preview smoke passed on $baseUrl."
