...
$appPath = Join-Path $repoRoot 'apps/timenexus'
$serverPath = Join-Path $appPath '.output/server/index.mjs'

if (-not (Test-Path $serverPath)) {
    throw "Nuxt server build not found at $serverPath. Run pnpm --filter @supersites/timenexus build first."
}
...
try {
    ...
    $homeHtml = (Invoke-PreviewRequest -Uri "$baseUrl/en" -RequiredContent 'TimeNexus' -Attempts 40 -TimeoutSec 3).Content
    ...
    $tool = Invoke-PreviewRequest -Uri "$baseUrl/en/tools/timezone-converter" -RequiredContent 'Time Zone Converter'
    if ($tool.StatusCode -ne 200 -or $tool.Content -notmatch 'FAQPage' -or $tool.Content -notmatch 'IANA time zones') {
    ...
    if ($privacy.StatusCode -ne 200 -or $privacy.Content -notmatch 'TimeNexus privacy') {
    ...
    Write-Host "TimeNexus preview smoke passed on $baseUrl."
