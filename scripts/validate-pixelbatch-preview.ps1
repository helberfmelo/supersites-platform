...
$appPath = Join-Path $repoRoot 'apps/pixelbatch'
$serverPath = Join-Path $appPath '.output/server/index.mjs'

if (-not (Test-Path $serverPath)) {
    throw "Nuxt server build not found at $serverPath. Run pnpm --filter @supersites/pixelbatch build first."
}
...
try {
    ...
    $homeHtml = (Invoke-PreviewRequest -Uri "$baseUrl/en" -RequiredContent 'PixelBatch' -Attempts 40 -TimeoutSec 3).Content
    ...
    $compressor = Invoke-PreviewRequest -Uri "$baseUrl/en/tools/image-compressor" -RequiredContent 'Image Compressor'
    if ($compressor.StatusCode -ne 200 -or $compressor.Content -notmatch 'FAQPage' -or $compressor.Content -notmatch 'Process image') {
    ...
    if ($sitemap.Content -notmatch '/en/privacy' -or $sitemap.Content -notmatch '/pt-br/tools/image-compressor' -or $sitemap.Content -notmatch '/de/tools/social-preset-generator') {
        throw 'Sitemap smoke failed.'
    }

    Write-Host "PixelBatch preview smoke passed on $baseUrl."
