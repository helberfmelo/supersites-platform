...
$appPath = Join-Path $repoRoot 'apps/docshift'
$serverPath = Join-Path $appPath '.output/server/index.mjs'

if (-not (Test-Path $serverPath)) {
    throw "Nuxt server build not found at $serverPath. Run pnpm --filter @supersites/docshift build first."
}
...
try {
    ...
    $homeHtml = (Invoke-PreviewRequest -Uri "$baseUrl/en" -RequiredContent 'DocShift' -Attempts 40 -TimeoutSec 3).Content
    ...
    $merge = Invoke-PreviewRequest -Uri "$baseUrl/en/tools/pdf-merge" -RequiredContent 'PDF Merge'
    if ($merge.StatusCode -ne 200 -or $merge.Content -notmatch 'FAQPage' -or $merge.Content -notmatch 'Process document') {
    ...
    if ($privacy.StatusCode -ne 200 -or $privacy.Content -notmatch 'DocShift privacy') {
    ...
    Write-Host "DocShift preview smoke passed on $baseUrl."
