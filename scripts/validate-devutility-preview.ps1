...
$appPath = Join-Path $repoRoot 'apps/devutility-lab'
$serverPath = Join-Path $appPath '.output/server/index.mjs'

if (-not (Test-Path $serverPath)) {
    throw "Nuxt server build not found at $serverPath. Run pnpm --filter @supersites/devutility-lab build first."
}
...
try {
    ...
    $homeHtml = (Invoke-PreviewRequest -Uri "$baseUrl/en" -RequiredContent 'DevUtility Lab' -Attempts 40 -TimeoutSec 3).Content
    ...
    $tool = Invoke-PreviewRequest -Uri "$baseUrl/en/tools/structured-data-formatter" -RequiredContent 'Structured Data Formatter'
    if ($tool.StatusCode -ne 200 -or $tool.Content -notmatch 'FAQPage' -or $tool.Content -notmatch 'JSON pretty print') {
    ...
    if ($sitemap.Content -notmatch '/en/privacy' -or $sitemap.Content -notmatch '/pt-br/tools/timestamp-converter' -or $sitemap.Content -notmatch '/de/tools/hash-generator') {
        throw 'Sitemap smoke failed.'
    }

    Write-Host "DevUtility Lab preview smoke passed on $baseUrl."
