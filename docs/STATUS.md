...
- Sprint 9.1 validation:
  ...
  - Workflows manuais de deploy receberam inputs opcionais `phase` e `sprint`; seus nomes de run e logs agora imprimem `Fase <phase> Sprint <sprint>`. O `Deploy Dry Run` tambem recebeu os inputs e imprime o contexto nos logs do job, mantendo o run-name padrao para compatibilidade com `push`. A convencao operacional foi registrada em `docs/RUNBOOKS/CI_CD.md`.
