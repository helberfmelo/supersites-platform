...
Na Sprint 9.16, o fechamento local/pre-push da Fase 9 adicionou um gate publico AdSense-safe (`pnpm validate:adsense-safe-public`), rotulagem configuravel no crawler benchmark e JSON-LD para homes/paginas estaticas dos 10 apps. O crawler full publico pre-correcao de 2026-06-28 (`2026-06-28T18-58-27-647Z`) navegou 876 rotas e 1752 checks desktop/mobile em `https://opentshost.com`, com 0 falhas de pagina/browser, 0 console errors, 0 links internos quebrados, 0 overflow horizontal, 0 gaps de robots/sitemap/title/meta/canonical/hreflang e 680 gaps de JSON-LD ainda visiveis na producao anterior. Esses gaps foram tratados localmente com schema `WebSite` nas homes e `WebPage` nas paginas estaticas amostradas; o crawler publico final sera reexecutado apos deploy dos apps. Lighthouse/PageSpeed/LHCI nao estavam disponiveis no PATH local, portanto o fechamento usou proxies deterministicos do crawler: TTFB mediano 33 ms, P75 load 86 ms, P75 LCP 88 ms e P75 CLS 0. O novo gate AdSense-safe passou em 13 paginas publicas, confirmando placeholders inertes (`blocked-consent`/`delivery-disabled`), sem requests/DOM de AdSense, GTM/GA4, pagamento, doacao ou afiliado. Builds/testes/Playwright/previews dos 10 apps, testes de pacotes, `validate:public-copy` em 891 HTMLs, estrutura, segredos, deploy dry-run, ci:changes e diff check passaram localmente. Remoto Quality Gate/Deploy Dry Run, deploys rotulados e smokes publicos finais ainda serao registrados apos push. Nenhum anuncio real, GTM/GA4, analytics externo, checkout, billing, pagamento, doacao, afiliado, worker/cron, API paga, snippet AdSense, `ads.txt`, publisher id real ou direct-root mapping foi ativado.
...
- Sprint 9.16 em fechamento local/pre-push:
  ...
  - Cobertura JSON-LD local foi adicionada para homes dos 10 apps (`WebSite`) e paginas estaticas aplicaveis (`WebPage` com `publisher`, canonical e `dateModified`), reduzindo a lacuna tecnica antes do deploy. A checagem direta dos HTMLs gerados confirmou `application/ld+json` em amostras de homes e paginas estaticas EN de NetProbe, CalcHarbor, DevUtility Lab, TimeNexus, QRRoute, InvoiceCraft, MailHealth, SitePulse Lab, PixelBatch e DocShift. O crawler full publico final sera reexecutado apos os deploys rotulados Fase 9/Sprint 9.16.
  - Lighthouse/PageSpeed/LHCI nao estavam disponiveis no PATH local. Nenhum pacote global ou provider externo foi ativado para isso; a sprint registrou os proxies deterministicos do crawler: TTFB mediano 33 ms, TTFB P75 38 ms, load mediano 77 ms, load P75 86 ms, LCP mediano 80 ms, LCP P75 88 ms e CLS P75 0.
  ...
  - Remoto Quality Gate, Deploy Dry Run, smokes publicos finais e fechamento docs-only ainda serao registrados apos push.
  - Esta sprint ainda nao ativou AdSense real, GTM/GA4, analytics externo, checkout, billing, pagamento, doacao, afiliado, conta, worker/cron, API paga, publicacao automatica, conteudo em massa, `ads.txt`, publisher id real, snippet `adsbygoogle` ou direct-root mapping. A lacuna operacional remanescente antes do fechamento e publicar os apps afetados, rodar smokes publicos, reexecutar o crawler full e registrar os run IDs finais.
