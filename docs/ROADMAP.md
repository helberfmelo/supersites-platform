...
Atualizacao local/pre-push de 2026-06-28 para Sprint 9.16: o crawler full publico pre-correcao `2026-06-28T18-58-27-647Z` navegou as mesmas 876 rotas e 1752 checks desktop/mobile com 0 falhas de pagina/browser, 0 console errors, 0 links internos quebrados, 0 overflow, 0 robots/sitemap gaps e 0 gaps de title/meta/canonical/hreflang. Ele ainda encontrou 680 gaps de JSON-LD na producao anterior, concentrados nas paginas estaticas dos apps; depois disso a sprint adicionou JSON-LD `WebSite` nas homes e `WebPage` nas paginas estaticas aplicaveis dos 10 apps, com smoke direto nos HTMLs gerados antes do deploy. Lighthouse/PageSpeed/LHCI nao estavam disponiveis no PATH local, entao a fase registrou proxies deterministicos de timing do crawler (P75 load 86 ms, P75 LCP 88 ms, CLS P75 0). O novo gate `pnpm validate:adsense-safe-public` passou em 13 paginas publicas e confirmou que espacos de ads seguem inertes e sem requests/DOM de AdSense, GTM/GA4, pagamento, doacao ou afiliados. O crawler full publico final sera reexecutado apos deploy dos apps afetados.
...
- Status: em fechamento local/pre-push. O gate `pnpm validate:adsense-safe-public` foi implementado e passou em 13 paginas publicas; `scripts/benchmark-crawl.mjs` agora aceita rotulos `--sprint`/`--symbolic-sprint`; o crawler full publico pre-correcao `2026-06-28T18-58-27-647Z` registrou 876 rotas, 1752 checks, 0 falhas, 0 links quebrados, 0 overflow, 0 gaps de robots/sitemap/title/meta/canonical/hreflang e 680 gaps de JSON-LD na producao anterior. A sprint adicionou JSON-LD para homes/paginas estaticas dos 10 apps e validou builds, testes, Playwright, previews, crawler smoke rotulado, public-copy em 891 HTMLs, estrutura, segredos, deploy dry-run, ci:changes, diff check e smoke direto de schema nos HTMLs gerados. Quality Gate/Deploy Dry Run remotos, deploys rotulados, smokes publicos finais e crawler full pos-deploy serao registrados apos push. Nenhum anuncio real, GTM/GA4, analytics externo, checkout, billing, pagamento, doacao, afiliado, worker, API paga, `ads.txt`, publisher id real ou snippet AdSense foi ativado.
