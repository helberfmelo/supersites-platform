...
| Ativar Stripe para doacoes, servicos ou planos pagos | O owner forneceu credenciais live para inventario/cofre e deploy de ambiente, e o receiver live de webhook Stripe ja esta implementado fail-closed. Ainda faltam endpoint/segredo `whsec_...`, DB de producao migrado/semeado, produtos/precos quando aplicavel, KYC, impostos, termos, politica de reembolso/cancelamento, smokes de provider e aprovacao por canal | Antes de publicar qualquer link de pagamento, botao de doacao real, checkout, widget, cobranca, assinatura, webhook live, invoice, refund ou entitlement pago via Stripe |
