...
const seoTitle = computed(() =>
  props.xDefault ? 'MailHealth' : limitSeoText(`${copy.value.title} | MailHealth`, SEO_TITLE_MAX_LENGTH),
)
...
<template>
  <main class="page-shell">
    ...
    <section class="hero" aria-labelledby="MailHealth-title">
      <div>
        <p class="eyebrow">{{ copy.eyebrow }}</p>
        <h1 id="MailHealth-title">{{ copy.title }}</h1>
        <p class="lead">{{ copy.lead }}</p>
      </div>

      <aside class="status-panel" aria-label="MailHealth status">
    ...
    <section class="tool-grid" aria-label="MailHealth tools">
    ...
    <MonetizationSafeBlock
      ...
      slot-id="mailhealth-home-footer-leaderboard"
    />
