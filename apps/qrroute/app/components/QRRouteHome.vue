...
useHead(() => ({
  ...
  title: props.xDefault ? 'QRRoute' : `${copy.value.title} | QRRoute`,
  meta: [
    {
      name: 'description',
...
<template>
  <main class="page-shell">
    ...
    <section class="hero" aria-labelledby="qrroute-title">
      ...
      <aside class="status-panel" aria-label="Sprint 4.1 status">
      ...
      <article v-for="tool in filteredTools" :key="tool.slug" class="tool-card">
        <div>
          <div class="tool-card__topline">
            ...
            <span :class="getStatusBadgeClass('foundation')">Sprint 4.1</span>
          </div>
