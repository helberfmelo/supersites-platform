...
const localizedBasics: Record<LocaleCode, {
  ...
  en: {
    ...
    localBody: 'The free tool uses browser processing when supported and keeps entered snippets out of product APIs.',
    qualitySection: 'Review method',
    qualityBody: 'Each utility exposes a focused transformation, visible limits and a short result that can be copied or inspected before any paid feature exists.',
    limitsSection: 'Limits',
    limitsBody: 'The free browser surface is designed for snippets and small files. Saved team areas, batch jobs, larger files and API automation are follow-on account options.',
    faqStorage: { question: 'Are snippets stored?', answer: 'No. DevUtility Lab does not use account storage, localStorage, sessionStorage or a product API in the free tool.' },
  ...
  'pt-br': {
    ...
    localBody: 'A ferramenta gratuita usa processamento local quando o navegador suporta e mantem trechos fora de APIs de produto.',
    ...
    limitsBody: 'A superficie gratuita foi feita para trechos e arquivos pequenos. Areas de equipe, lotes, arquivos maiores e automacao por API ficam como opcoes futuras de conta.',
    faqStorage: { question: 'Os trechos sao armazenados?', answer: 'Nao. O DevUtility Lab nao usa conta, localStorage, sessionStorage nem API de produto na ferramenta gratuita.' },
  ...
  es: {
    ...
    localBody: 'La herramienta gratis usa procesamiento local cuando el navegador lo soporta y mantiene fragmentos fuera de APIs de producto.',
    ...
    limitsSection: 'Limites',
    limitsBody: 'La superficie gratis es para fragmentos y archivos pequenos. Espacios de equipo, lotes, archivos mayores y automatizacion por API quedan como opciones futuras de cuenta.',
    faqStorage: { question: 'Se almacenan fragmentos?', answer: 'No. DevUtility Lab no usa cuentas, localStorage, sessionStorage ni API de producto en la herramienta gratuita.' },
  ...
  fr: {
    ...
    localBody: 'L outil gratuit utilise un traitement local quand le navigateur le supporte et garde les extraits hors des API produit.',
    ...
    limitsSection: 'Limites',
    limitsBody: 'La surface gratuite vise extraits et petits fichiers. Espaces equipe, lots, gros fichiers et automatisation API restent des options futures de compte.',
    faqStorage: { question: 'Les extraits sont-ils stockes?', answer: 'Non. DevUtility Lab n utilise ni compte, ni localStorage, ni sessionStorage, ni API produit dans l outil gratuit.' },
  ...
  de: {
    ...
    localBody: 'Das kostenlose Tool nutzt lokale Verarbeitung, wenn der Browser es unterstuetzt, und sendet Ausschnitte nicht an Produkt-APIs.',
    ...
    limitsBody: 'Die kostenlose Browser-Oberflaeche ist fuer Ausschnitte und kleine Dateien. Team-Bereiche, Stapel, groessere Dateien und API-Automation bleiben kuenftige Kontooptionen.',
    faqStorage: { question: 'Werden Ausschnitte gespeichert?', answer: 'Nein. DevUtility Lab nutzt im kostenlosen Tool weder Konto, localStorage, sessionStorage noch Produkt-API.' },
...
const specs: ToolSpec[] = [
  {
    ...
    upgradeScope: 'Saved team areas, larger files, batch jobs and API formatting.',
  ...
  },
  {
    ...
    title: 'Regex Tester',
    shortName: 'Regex',
    headline: 'Run JavaScript regular expressions against sample text with browser processing.',
    ...
    upgradeScope: 'Saved test suites, shared team areas, larger fixtures and API.',
  ...
  },
  {
    ...
    upgradeScope: 'Shared team areas, file diffing, batch comparisons and API.',
    exampleBody: 'The example compares two short blocks and marks changed, added and unchanged lines in a compact unified view.',
    commonErrorBody: 'Very large files are intentionally blocked in the free tool; use short snippets and redact confidential text.',
...
function ensureSnippetLimit(...values: string[]): void {
  ...
  if (totalLength > 200_000) {
    throw new Error('Free browser tools accept snippets up to 200 KB. Larger files belong in account features.')
  }
}
