...
export interface DevUtilityAnalyticsInput {
  toolSlug: ToolSlug
  locale: LocaleCode
  routePath: string
}

export function createDevUtilityToolEvent(
  input: DevUtilityAnalyticsInput,
...
export function pushDevUtilityAnalyticsEvent(event: AnalyticsEvent): boolean {
...
export function trackDevUtilityEvent(
  input: DevUtilityAnalyticsInput,
  name: 'tool_started' | 'tool_completed' | 'tool_failed',
): boolean {
  return pushDevUtilityAnalyticsEvent(createDevUtilityToolEvent(input, name))
}
