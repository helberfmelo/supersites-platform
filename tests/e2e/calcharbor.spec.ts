...
test.describe('CalcHarbor MVP', () => {
  test('renders the home page on desktop', async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: 'All calculators' })).toBeVisible()
  ...
  test('calculates a loan payment and records sanitized analytics', async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: 'Principal vs interest' })).toBeVisible()
    ...
    await expect(page.getByRole('button', { name: 'Compare scenarios' })).toBeVisible()
    ...
    await expect(page.getByRole('heading', { name: 'Save and export for repeat work' })).toBeVisible()
  ...
  test('renders a localized ROI calculator and policy page on mobile', async ({ page }, testInfo) => {
    ...
    await expect(page).toHaveTitle(/CalcHarbor privacy/)
    await expect(page.getByRole('heading', { level: 1 })).toHaveText('CalcHarbor privacy')
